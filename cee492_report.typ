...
#v(1.0em)
This study is motivated by the potential to apply data science principles to the critical infrastructure issue of bridge foundation scour. The primary aim is to leverage modern analytical techniques to gain new insights into scour behavior, thereby enhancing the monitoring, mitigation, and protection of vital public assets from this problem. More broadly, the project seeks to demonstrate the value that contemporary data science can bring to traditional civil engineering challenges.
In this context, the project's central research hypothesis is that analyzing historical streambed profile data, when processed using data science methodologies, can reveal detectable patterns and key indicators related to scour behavior at specific bridge locations, provided that the necessary, albeit rudimentary, data is collected. This study specifically investigates this hypothesis by applying the Julia programming language to analyze streambed profile data supplied by the USGS for select bridges in the state of Montana. While the precise scope of the project evolved during the Exploratory Data Analysis phase, the fundamental objective remained unchanged: to use data science to uncover actionable insights from hydrological and structural data related to bridge scour.
