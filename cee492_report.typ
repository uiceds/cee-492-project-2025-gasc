...
From the results presented in Table VI and by referencing the respective figures (Figure 5 and 6) for each structure, it was apparent that of the three structures, the structures with the least variation year to year had more accurate model predictions than the more erratic years. Structure _S00370 000+0.5361_, both in the upstream and downstream sides, has the most erratic streambed profile year over year. The predicted profiles for this structure had the lowest $R^2$ value, which is supported by the previous statement regarding the erratic streambed elevations. 
#v(0.3em)
While the $R^2$ value differed per structure, something that remained relatively consistent was the RMSE for all observations made. The column in Table VI is reported in feet and represents the magnitude of inaccuracy between the predicted and actual profile at any given point. 

= Discussion

== Hypothesis Assessment

The initial hypothesis theorized that analyzing historical streambed profile data using data science methodologies could reveal detectable patterns and key indicators related to scour behavior. The study's findings partially support this hypothesis. The application of SVD combined with Ridge Regularization effectively decomposed complex riverbed profiles into dominant behaviors, enabling the prediction of general channel evolution. When solely utilizing the coefficient of determination($R^2$) in assessments, the model appeared to demonstrate reasonable accuracy, with $R^2$ values ranging from 0.55 to 0.90 across different structures, indicating a capability to capture significant morphological trends. Additionally, it can be reasoned that the variance in $R^2$ values across structures highlights the heterogeneity of model performance. The introduction of RMSE in assessment allowed for further interpretation of the model’s accuracy. For example, in terms of the first two structures presented in Table VI (P00007 043+0.666 & S00373000+04001), the $R^2$ appears significantly higher for the downstream data when compared to the upstream. However, the RMSE across this range shows much less variability. This discrepancy suggests that while the model captures general trends, it faces limitations in resolving abrupt, localized features—such as deep scour holes or steep bank transitions—and the current predictive capabilities of the model do not yet encompass all aspects of scour phenomenology. Ultimately, while this study confirms that data science methodologies can uncover meaningful insights into scour behavior, it emphasizes the necessity for further refinement and the integration of exogenous data sources to enhance predictive fidelity.
...
Two seemingly simple but impactful challenges that were faced involved integrating pier locations and standardizing cross-sectional data widths. Pier data is available but will take effort to prepare in a way that it can integrate into this model. Knowing the locations of piers within the cross sections could provide critical context for interpreting scour patterns. Similarly, the horizontal extents of cross-sectional data varied, which undoubtedly complicated the model's ability to assess and learn patterns and prevented the potential analysis of stream edge scour. 

#v(0.3em)

=== Relation to Stream Gauge Data

Another potential step for this line of research is drawing connection between the stream gauge data, observed streambed elevation data, and the predictive model. As discussed previously, the USGS keeps vast amounts of stream data, which add key context to when and why given streams may experience scour. The natural step forward with this research is to find more input variables for the model to analyze. 
