...
To develop the predictive model, the project team evaluated whether to utilize the entire streambed cross-section or focus on localized pier data. Concurrent with scope adjustments, the team determined that integrating specific pier locations with the broader streambed data was effectively unfeasible. Therefore, the SVD/PCA analyses was conducted exclusively on the streambed geometry data.

To manage the dimensionality of the dataset, the team narrowed the scope to bridges located along a single watercourse. Prioritizing a river with abundant, high-quality historical data was critical to this approach. Preliminary analysis identified the Bitterroot River as a prime candidate, featuring three distinct structures with over five years of observation data. While the team considered an alternative approach—developing a single-bridge model using a temporal training split (e.g., 2012–2020) for validation—the multi-bridge analysis of the Bitterroot River was selected as the superior strategy.
